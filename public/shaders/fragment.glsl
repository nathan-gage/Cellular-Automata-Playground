...
void main(void) {
	if(doStep) {
		...
		float sum = texture2D(u_image, getCoords(texCoord, vec2(1., -1.))).a * u_kernel[0] + texture2D(u_image, getCoords(texCoord, vec2(0., -1.))).a * u_kernel[1] + texture2D(u_image, getCoords(texCoord, vec2(-1., -1.))).a * u_kernel[2] + texture2D(u_image, getCoords(texCoord, vec2(1., 0.))).a * u_kernel[3] + texture2D(u_image, getCoords(texCoord, vec2(0., 0.))).a * u_kernel[4] + texture2D(u_image, getCoords(texCoord, vec2(-1., 0.))).a * u_kernel[5] + texture2D(u_image, getCoords(texCoord, vec2(1., 1.))).a * u_kernel[6] + texture2D(u_image, getCoords(texCoord, vec2(0., 1.))).a * u_kernel[7] + texture2D(u_image, getCoords(texCoord, vec2(-1., 1.))).a * u_kernel[8];
